...
package com.ihujia.hujia.store.maputils.view;
...
import com.ihujia.hujia.store.maputils.IconGenerator;
import com.ihujia.hujia.store.maputils.MarkerManager;
import com.ihujia.hujia.store.maputils.Point;
import com.ihujia.hujia.store.maputils.SphericalMercatorProjection;
import com.ihujia.hujia.store.maputils.SquareTextView;
...
import static com.ihujia.hujia.store.maputils.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
	...
	private class RenderTask implements Runnable {
		...
		public void run() {
			...
			// markers to animate from.
			List<Point> newClustersOnScreen;
	...
	private class CreateMarkerTask {
		...
		private void perform(MarkerModifier markerModifier) {
			...
			if (!shouldRenderAsCluster(cluster)) {
				for (T item : cluster.getItems()) {
					...
					if (marker == null) {
						...
						//修改聚合点展开后的marker锚点，可以尝试获取之前的
						onBeforeClusterItemRendered(item, markerOptions.anchor(0.15f, 0.8f));
			...
			//修改聚合点展开后的marker锚点，可以尝试获取之前的
			Marker marker = mClusterManager.getClusterMarkerCollection().addMarker(markerOptions.anchor(0.15f, 0.8f));
